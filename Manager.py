...
def collect_image_paths(folder, image_extensions):
    ...
    Returns (paths, skipped) where skipped is a list of (path, reason) tuples.
    """
    paths = []
    skipped = []
    ...
    for item in items:
        if item.is_file():
            ...
            sub_paths, sub_skipped = collect_image_paths(item, image_extensions)
            paths.extend(sub_paths)
            skipped.extend(sub_skipped)
            if sub_paths:
                print(f"    {item.name}/  →  {len(sub_paths)} image(s)")
